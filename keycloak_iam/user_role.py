...
assign_roles_to_exiting_users(
    ...
    role_ids=[trusted_guest_role.id, airflow_trusted_viewer_role.id, airflow_viewer_role.id, grafana_editor_role.id],
    users=config.trusted_guest_users,
)
assign_roles_to_exiting_users(
    realm_id=main_realm.realm,
    provider=master_provider,
    role_ids=[admin_role.id, airflow_admin_role.id, grafana_admin_role.id],
